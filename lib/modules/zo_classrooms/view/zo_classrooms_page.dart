...
class ZoClassroomsPage extends StatelessWidget {
  const ZoClassroomsPage({super.key});
  ...
  Widget _body(ZoClassroomsState state) {
    if (state is ZoClassroomInitial) {
      return const Center(child: CircularProgressIndicator());
    }

    if (state is ZoClassroomsLoading) {
      return Center(
        child: Column(
    ...
    if (state is ZoClassroomsLoaded) {
      return Column(
        children: [
  ...
  Widget? _drawer(ZoClassroomsState state, BuildContext context) {
    if (state is ZoClassroomsLoaded) {
      return Drawer(
        child: Padding(
  ...
  Widget _appBarText(ZoClassroomsState state) {
    return Text(state.appBarTitle ?? 'Аудитории', maxLines: 2);
  }

  Widget _dropDownButton(ZoClassroomsLoaded state) {
    return Padding(
